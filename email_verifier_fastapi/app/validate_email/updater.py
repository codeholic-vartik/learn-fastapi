...
class BlacklistUpdater(object):
    ...
    def _download(self, headers: dict, blacklist_path: Path, etag_path: Path):
        'Download and store the blacklist file.'
        context = ssl._create_unverified_context()  # Create an unverified SSL context
        for url in BLACKLIST_URL:
            try:
                LOGGER.debug(msg=f'Checking {url}')
                request = Request(url=url, headers=headers)
                response: HTTPResponse = urlopen(url=request, context=context)  # Pass context
                # New data available
                ...
                LOGGER.warning(msg=f'Failed to download from {url}: {exc}')
        raise Exception('All URLs in BLACKLIST_URL failed.')
